...
topomon_wifi_if_down() {
	...
	if [ -n "$sta_ifname" ]; then
		...
		if [ "$sta_disabled" == "0" ]; then
			log "topomon_wifi_if_down: uci set $sta_ifname disabled=1"
			...
			log "topomon_wifi_if_down: ifconfig $sta_ifname set ApCliEnable=0"
			ifconfig $sta_iface set ApCliEnable=0
			sleep 3
...
topomon_wifi_if_up() {
	...
	if [ -n "$sta_ifname" ]; then
		...
		if [ "$sta_disabled" == "1" ]; then
			...
			local sta_ssid=$(uci -q get wireless.$sta_iface.ssid)
			local sta_key=$(uci -q get wireless.$sta_iface.key)
			iwpriv $sta_ifname set ApCliEnable=0
			iwpriv $sta_ifname set ApCliMWDS=1
			iwpriv $sta_ifname set ApCliAuthMode=WPA2PSK
			iwpriv $sta_ifname set ApCliEncrypType=AES
			iwpriv $sta_ifname set ApCliSsid="$sta_ssid"
			iwpriv $sta_ifname set ApCliWPAPSK="$sta_key"
			iwpriv $sta_ifname set ApCliSsid="$sta_ssid"
...
topomon_update_cap_wifi_param() {
	...
	if [ $wait -eq 1 ]; then
		for i in {1..15}
		do
			ifconfig $bh_wlan_iface |grep UP > /dev/null 2>&1
			if [ $? -eq 0 ]; then
				break
			else
				sleep 2
...
trigger_dhcp_new_ip(){
	...
	[ "$curr_bh_type" != "wired" ] && {
		local bh_sta_linkup=$(ifconfig $bh_sta_ifname | grep -wc "RUNNING")
		[ "$bh_sta_linkup" == "0" ] && {
			log "trigger_dhcp_new_ip: $bh_sta_iface not linkup, set ApCliAutoConnect first"
			iwpriv $bh_sta_ifname set ApCliAutoConnect=3
			return
		}
	}
...
notify_wifi_bh_linked(){
	...
	local bh_ap_iface=$(uci -q get misc.backhauls.backhaul_${bh_band}_ap_iface)
	hop_count=$(iwpriv $bh_sta_iface get mesh_hop | awk -F":" '{print $2}')
	log "notify_wifi_bh_linked hop_count = $hop_count"
	if [ $hop_count != "255" ]; then
		local ulmac=`mtknetlink_cli ${bh_ap_iface} mesh_ulmac|awk '{print $2}'`
		log "notify_wifi_bh_linked -> ubus call topomon wifi_up ulmac:$ulmac"
...
bhsta_link_check() {
	local bh_sta_ifname=$(uci -q get misc.backhauls.backhaul_5g_sta_iface)
	local link_status=`iwpriv $bh_sta_ifname Connstatus|grep Connected > /dev/null;echo $?`
